...
export default function useWeb3() {
  ...
  const getMainContract = async () => {
    console.log(chainId);
    var contract = Object.entries(contracts).find(([key, value]) =>
      parseInt(key) == chainId ? value : null
    )[1];
  ...
  const createCCO = async ({
    ...
    var c1 = Object.entries(contracts).find(([key, value]) =>
      parseInt(key) == chainId ? value : null
    )[1];
  ...
  const getCCO = async ({ tokenAddress }) => {
  ...
  const invest = async ({ tokenAddress, usdcAmount }) => {
  ...
  const getTokenName = async ({ tokenAddress }) => {
    var usdcContract = getERC20Contract();
    return await usdcContract.symbol();
  };
