...
export default function useWeb3() {
    ...
    const changeNetwork = async ({ chainId }) => {
        if (window.ethereum.networkVersion !== chainId) {
            try {
                await window.ethereum.request({
                    ...
                    params: [{ chainId: "0x" + new ethers.hexlify(ethers.toUtf8Bytes(chainId.toString())) }]
                });
            } catch (err) {
                ...
                if (err.code === 4902) {
                    await window.ethereum.request({
                        ...
                        params: [
                            { ...networks[chainId], chainId: "0x" + new ethers.hexlify(ethers.utils.toUtf8Bytes(chainId.toString())) }
                        ]
                    });
                }
            }
        ...
        try {
            await window.ethereum.request({
                method: 'wallet_switchEthereumChain',
                params: [{ chainId: "0x" + new ethers.hexlify(ethers.utils.toUtf8Bytes(chainId.toString())) }],
            })
        } catch (error) {
            console.debug(
